...
className='flex flex-wrap justify-center mt-16 gap-3'>
  {Array(6).fill('').map((_, index) => {
    const imageIndex = (index % 5) + 1; // cycles 1 → 5 repeatedly
    return (
      <motion.img
      whileHover={{ scale: 1.05, duration: 0.1}}
        key={index}
        className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10'
        src={assets[`sample_img_${imageIndex}`]}
        alt={`sample_img_${imageIndex}`}
        width={70}
      />
    );
  })}
    ...
    className='mt-2 text-neutral-600'>Generated Images from ImagineRise</motion.p>
    </motion.div>
  )
}
